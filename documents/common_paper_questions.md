...
* **Q: Source and permissions to release data.**
    ...
    We have now elaborated on the source and permissions in Table 3 ([supplementary](https://www.cs.jhu.edu/~alanlab/Pubs23/qu2023abdomenatlas.pdf)). As said, all the CT volumes are from public resources. This means we may not be able to distribute their original CT volumes without the permission from the authors. We will check with each one of them individually. Before that, we will start to disseminate the organ masks, and users will have to retrieve the original CT volumes, if needed, from the original sources (websites). Everything we intend to create and license-out will be in separate files and no modifications are necessary to the original CT volumes. We have consulted this with the lawyers at Johns Hopkins University, confirming the permissions of distributing the annotations based on the license of each dataset.

* **Q: Comparing AbdomenAtlas-8K with AMOS, AbdomenCT-1K, TotalSegmentator**

    1. **A significantly larger number of annotated CT volumes.** TotalSegmentator, AMOS, and AbdomenCT-1K provided 1,204, 500, and 1,112 annotated CT volumes. AbdomenAtlas-8K provided 8,448 annotated CT volumes (around eight times larger).
...
* **Q: Could we please clarify the source and permissions for releasing the data? It would also be helpful to understand the information about data reuse from previous datasets.** 
...
* **Q: It might be beneficial if the authors consider presenting a comparison, possibly in the form of a table, showcasing the source countries of data from previous datasets alongside the proposed AbdomenAtlas-8K.**
...
* **Q:It's noteworthy that while the authors initially highlight concerns about the TotalSegmentator's use of a single-model automatic segmentation, their approach also leans towards automation, albeit with an ensemble of three models, and manual annotation is done for just 400 out of the 8,000 volumes.**
...
* **Q: Reducing abdominal labels to 8 omits smaller, clinically-relevant structures like adrenal glands. Compared to TotalSegmentator's 104 anatomies, this seems less comprehensive.**
...
* **Q: It would be helpful to clarify if all the data, especially the Abdomen8k subset from JHH, will be made public upon acceptance given the licensing note.**
...
* **Q: It would be helpful to further explore the relationship with TotalSegmentator. Is it just the single nnUNet model that causes the high correlation, or would an ensemble of three nnUNets also be effective?**
...
* **Q: It would be insightful to understand the strengths and areas for improvement when comparing Abdomen-8K to TotalSegmentator.**
...
* **Q: It might be helpful to provide a definition or context when referring to the JHH dataset.**
...
* **Q: It would be beneficial to offer a clear definition or explanation for "pseudo labels."**
...
* **Q: From the study, it appears three human annotators were involved. It would be helpful to understand the method used for their assignment.**
...
* **Q:The inter-annotator agreement seems to vary significantly for certain organs. It'd be helpful to understand if the initial AI annotation might have influenced the human annotators' decisions in any way.**
